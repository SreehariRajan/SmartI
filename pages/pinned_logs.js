    ...
    useEffect(() => {
        (async () => {
            if (currentAccount) {
                ...
                const data = await casesContract.getPinnedCases(currentAccount);
            ...
            <div className={styles.table}>
                ...
                {(logs.length > 0 && !loading) ?
                    filteredLogs.map((log, index) => {
                        return (
                            <Log key={index} id={log.id} slno={index + 1} location={log.location} time={log.time} date={log.date} type={log.category} verified={log.verified} />
...
const styles = {
    ...
    table: 'rounded-3xl mt-5 shadow-xl w-3/4 flex flex-col backdrop-blur-3xl  bg-slate-500 bg-opacity-10 drop-shadow-2xl py-5  mb-10 overflow-y-auto items-center',
